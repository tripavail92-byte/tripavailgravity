...
import { 
    Luggage, Heart, User, Settings, ChevronRight, 
    Calendar, MapPin, BadgeCheck, Compass, ArrowRight
} from 'lucide-react';
                ...
                <div className="max-w-5xl mx-auto">
                    ...
                    >
                        <h1 className="text-3xl md:text-4xl font-black text-gray-900 mb-2">
                            Welcome back, <span className="text-primary">{user?.user_metadata?.full_name?.split(' ')[0] || 'Traveler'}</span>!
                        </h1>
            ...
            <div className="max-w-5xl mx-auto px-4 -mt-12">
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div className="lg:col-span-2 space-y-8">
                        ...
                        >
                            <GlassCard variant="card" className="p-0 overflow-hidden rounded-3xl border-none shadow-xl shadow-gray-200/50">
                                <div className="p-6 border-b border-gray-50 flex items-center justify-between">
                                    <h2 className="text-lg font-bold text-gray-900 flex items-center gap-2">
                                        <Luggage className="w-5 h-5 text-primary" />
                                        Next Adventure
                                    </h2>
                                    <Link to="/trips" className="text-xs font-bold text-primary uppercase tracking-widest hover:underline">
                                        View All
                                    </Link>
                                </div>
                                {isLoading ? (
                                    ...
                                    <div className="p-6 flex flex-col md:flex-row gap-6">
                                        ...
                                        <div className="flex-1">
                                            <h3 className="text-xl font-black text-gray-900 mb-2">
                                                {(nextTrip.tours || nextTrip.packages)?.title || (nextTrip.tours || nextTrip.packages)?.name}
                                            </h3>
                                            <div className="flex flex-wrap gap-4 text-sm text-gray-500 font-medium">
                                                <div className="flex items-center gap-1.5">
                                                    <Calendar className="w-4 h-4 text-gray-400" />
                                                    {format(new Date(nextTrip.booking_date || nextTrip.start_time || nextTrip.check_in_date), 'MMM dd, yyyy')}
                                                </div>
                                                <div className="flex items-center gap-1.5">
                                                    <MapPin className="w-4 h-4 text-gray-400" />
                                                    {(nextTrip.tours || nextTrip.packages)?.location?.city || (nextTrip.tours || nextTrip.packages)?.location || 'TBA'}
                                                </div>
                                            </div>
                        ...
                        >
                            <GlassCard variant="card" className="p-0 overflow-hidden rounded-3xl border-none shadow-xl shadow-gray-200/50">
                                <div className="p-6 border-b border-gray-50 flex items-center justify-between">
                                    ...
                                    <Link to="/wishlist" className="text-xs font-bold text-primary uppercase tracking-widest hover:underline">
                                        View All
                                    </Link>
                                </div>
                                <div className="p-6">
                                    {recentWishlist.length > 0 ? (
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                            {recentWishlist.map((item) => (
                                                <Link 
                                                    key={item.id} 
                                                    to={item.type === 'tour' ? `/tours/${item.slug || item.id}` : `/packages/${item.slug || item.id}`}
                                                ...
                                                >
                                                    <div className="w-16 h-16 rounded-xl overflow-hidden shadow-sm flex-shrink-0">
                                                        <img 
                                                            src={item.images?.[0] || item.cover_image || 'https://images.unsplash.com/photo-1469474968028-56623f02e42e'} 
                                                    ...
                                                    <div className="flex-1 min-w-0">
                                                        ...
                                                        <p className="text-xs text-gray-500 flex items-center gap-1">
                                                            ...
                                                            {typeof item.location === 'string' ? item.location : item.location?.city || 'Globe'}
                                                        </p>
                                                    </div>
                    ...
                    <div className="space-y-8">
                        ...
                        >
                            <GlassCard variant="card" className="p-6 rounded-3xl border-none shadow-xl shadow-gray-200/50 bg-primary/5">
                                ...
                                <p className="text-sm text-gray-500 mb-6 font-medium">Complete your profile to unlock all features.</p>
                                ...
                                <Button asChild variant="outline" className="w-full rounded-2xl h-12 font-bold bg-white border-none shadow-sm hover:shadow-md transition-all">
                        ...
                        >
                            <h3 className="text-xs font-black text-gray-400 uppercase tracking-widest mb-4 ml-2">Quick Navigation</h3>
                            <div className="space-y-3">
                                <QuickLink to="/trips" icon={Luggage} label="My Bookings" color="bg-emerald-50 text-emerald-600" />
                                <QuickLink to="/wishlist" icon={Heart} label="Wishlist" color="bg-rose-50 text-rose-600" />
                                <QuickLink to="/settings" icon={Settings} label="Settings" color="bg-blue-50 text-blue-600" />
                            </div>
                        </motion.div>
                    </div>
                </div>
...
function QuickLink({ to, icon: Icon, label, color }: { to: string, icon: any, label: string, color: string }) {
    return (
        <Link to={to}>
            <GlassCard variant="light" className="p-4 rounded-2xl border-none shadow-sm hover:shadow-md hover:bg-white transition-all flex items-center justify-between group">
