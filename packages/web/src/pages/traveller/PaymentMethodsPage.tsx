...
import { 
    ...
    Smartphone, ShieldCheck, Info, Loader2
} from 'lucide-react';
import { useAuth } from '@/hooks/useAuth';
import { paymentMethodService, type UserPaymentMethod } from '@/features/traveller/services/paymentMethodService';
...
export default function PaymentMethodsPage() {
    ...
    const renderCard = (method: UserPaymentMethod) => (
        ...
        >
            <GlassCard variant="card" className="p-6 rounded-2xl h-full flex flex-col justify-between border-none shadow-lg overflow-hidden">
                {/* Background Pattern */}
                ...
                <div className="relative z-10">
                    <div className="flex justify-between items-start mb-6">
                        <div className={`p-3 rounded-xl ${method.method_type === 'card' ? 'bg-blue-50 text-blue-600' : 'bg-emerald-50 text-emerald-600'}`}>
                            {method.method_type === 'card' ? <CreditCard className="w-6 h-6" /> : <Smartphone className="w-6 h-6" />}
                        </div>
                        {method.is_default && (
                            <Badge className="bg-primary/10 text-primary border-none text-[10px] uppercase font-black px-2">Primary</Badge>
                        )}
                    </div>
                ...
                <div className="max-w-5xl mx-auto flex flex-col md:flex-row md:items-end justify-between gap-6">
                    ...
                    >
                        <div className="flex items-center gap-3 mb-4">
                            ...
                            <span className="text-xs font-black text-primary uppercase tracking-widest">Secure Payments</span>
                        </div>
            ...
            <div className="max-w-5xl mx-auto px-4 -mt-12">
                {isLoading ? (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {[1, 2, 3].map(i => (
                            <div key={i} className="h-48 rounded-3xl bg-white/50 animate-pulse border border-gray-100" />
                        ))}
                    </div>
                ) : methods.length > 0 ? (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    ...
                    <GlassCard variant="light" className="p-20 text-center rounded-[3rem] border-dashed border-2 border-gray-200 bg-transparent">
                        <div className="w-20 h-20 bg-gray-100 rounded-full flex items-center justify-center mx-auto mb-6">
                            <CreditCard className="w-10 h-10 text-gray-300" />
                        </div>
                        <h2 className="text-xl font-bold text-gray-900 mb-2">No payment methods saved</h2>
                        <p className="text-gray-500 max-w-sm mx-auto mb-8 font-medium">Add a card or mobile wallet to enjoy seamless bookings on TripAvail.</p>
                        <Button variant="outline" onClick={() => setIsAddingCard(true)} className="rounded-full px-8">
                            Get Started
                        </Button>
                    </GlassCard>
                )}
                ...
                <div className="mt-12 p-6 rounded-3xl bg-blue-50/50 border border-blue-100 flex gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-xs text-blue-700/80 font-medium leading-relaxed">
                            TripAvail uses Stripe for international credit card processing. We never store your full card details on our servers. 
                            Mobile wallet transactions are secured via dedicated provider APIs.
                        </p>
                    </div>
                </div>
            </div>
                                ...
                                {activeTab === 'card' ? (
                                    <div className="space-y-6">
                                        <div className="p-8 rounded-3xl bg-gray-50 border border-gray-100 text-center">
                                            ...
                                            <p className="text-sm text-gray-500 font-medium mb-6">Secure Stripe integration will appear here for card entry.</p>
                                            <Button 
