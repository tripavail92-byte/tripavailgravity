...
export function PropertyOwnershipSubFlow({ onComplete, initialData }: PropertyOwnershipSubFlowProps) {
    const { user } = useAuth();
    ...
    const [propertyLivePhotoUrl, setPropertyLivePhotoUrl] = useState(initialData?.propertyLivePhotoUrl || '');
    ...
    const handleUpload = async (file: File, type: 'deed' | 'bill' | 'live') => {
        ...
        setIsUploading(true);
        try {
            const url = await hotelManagerService.uploadAsset(user.id, file, `verification/property/${type}`);
            if (type === 'deed') {
                setTitleDeedUrl(url);
                setStep('bill');
            } else if (type === 'bill') {
                ...
                {step === 'deed' && (
                    ...
                    >
                        <div className="text-center">
                            ...
                            <h4 className="text-2xl font-black text-gray-900 tracking-tight italic uppercase">Property Ownership</h4>
                            <p className="text-gray-500 mt-2 font-medium">Please upload your Property Deed or Lease Agreement to prove ownership or management rights.</p>
                        </div>
                ...
                {step === 'bill' && (
                    ...
                    >
                        <div className="text-center">
                            ...
                            <h4 className="text-2xl font-black text-gray-900 tracking-tight italic uppercase">Address Verification</h4>
                            <p className="text-gray-500 mt-2 font-medium">Upload a recent Utility Bill (Electricity, Water, or Fiber) showing the property address.</p>
                        </div>
                        ...
                        <button onClick={() => setStep('deed')} className="w-full text-center text-xs font-bold text-gray-400 uppercase tracking-widest hover:text-primary">← Go Back</button>
                    </motion.div>
                )}
                ...
                {step === 'live' && (
                    ...
                    >
                        <div className="text-center">
                            ...
                            <h4 className="text-2xl font-black text-gray-900 tracking-tight italic uppercase">Live Verification</h4>
                            <p className="text-gray-500 mt-2 font-medium">Take a real-time photo of the hotel exterior or main signage to prove you are physically present at the property.</p>
                        </div>
                        ...
                        <button onClick={() => setStep('bill')} className="w-full text-center text-xs font-bold text-gray-400 uppercase tracking-widest hover:text-primary">← Go Back</button>
                    </motion.div>
                )}
                ...
                {step === 'summary' && (
                    ...
                    >
                        <Card className="p-10 rounded-[32px] text-center bg-primary/[0.02] border-primary/10 border-2">
                            ...
                            <h4 className="text-3xl font-black text-gray-900 tracking-tight italic uppercase italic">Property Verified</h4>
                            <p className="text-gray-500 mt-4 font-medium px-4">
                                All ownership and location evidence has been collected. Our team will verify these against local records.
                            </p>
