import { BedDouble, DollarSign, Percent, Plus, Trash2, Users } from 'lucide-react'
...
export function TourPricingStep({
  ...
  const handleCurrencyChange = (nextCurrency: string) => {
    ...
    const convertedTiers = pricingTiers.map((tier: any) => ({
      ...
      pricePerPerson: Math.round(
        convertedPrice * (1 - (Number(tier.discountPercentage) || 0) / 100),
      ),
    }))
  ...
  const hasSavedDepositPercentage =
    Number.isFinite(rawDepositPercentage) && rawDepositPercentage > 0
  ...
  // Accommodation / room-sharing pricing (multi-day tours). Stored on data.accommodation_pricing;
  // the traveller booking card renders a sharing selector + traveller counters from it.
  const accommodationData = (data.accommodation_pricing as any) || {}
  const accEnabled = Boolean(accommodationData.enabled)
  const accTiers: Array<{ key: string; label: string; pricePerPerson: number }> = Array.isArray(
    accommodationData.tiers,
  )
    ? accommodationData.tiers
    : []
  const accChildRates = {
    withBed: Number(accommodationData.childRates?.withBed ?? 0.8),
    noBed: Number(accommodationData.childRates?.noBed ?? 0.6),
    infant: Number(accommodationData.childRates?.infant ?? 0),
  }
  const ROOM_TIER_DEFS = [
    { key: 'quad', label: 'Quad sharing', hint: '4 per room — cheapest per person' },
    { key: 'triple', label: 'Triple sharing', hint: '3 per room' },
    { key: 'double', label: 'Double sharing', hint: '2 per room' },
    { key: 'solo', label: 'Solo', hint: '1 per room — most expensive' },
  ]
  const tierPriceFor = (key: string) => accTiers.find((t) => t.key === key)?.pricePerPerson ?? 0
  const writeAccommodation = (patch: {
    enabled?: boolean
    tiers?: typeof accTiers
    childRates?: typeof accChildRates
  }) => {
    onUpdate({
      accommodation_pricing: {
        enabled: patch.enabled ?? accEnabled,
        tiers: patch.tiers ?? accTiers,
        childRates: patch.childRates ?? accChildRates,
      },
    } as Partial<Tour>)
  }
  const setTierPrice = (key: string, label: string, price: number) => {
    const others = accTiers.filter((t) => t.key !== key)
    const nextTiers = price > 0 ? [...others, { key, label, pricePerPerson: price }] : others
    writeAccommodation({ tiers: nextTiers })
  }
  const setChildRatePct = (key: 'withBed' | 'noBed' | 'infant', pct: number) => {
    const frac = Math.max(0, Math.min(100, Number(pct) || 0)) / 100
    writeAccommodation({ childRates: { ...accChildRates, [key]: frac } })
  }
  ...
  const toggleInclude = (item: string) => {
    ...
    const includedFeatures = INCLUDED_FEATURE_OPTIONS.filter((feature) =>
  ...
  const handleDepositPercentageChange = (value: number) => {
    handleInputChange(
      'deposit_percentage',
      Math.max(effectiveMinimumDeposit, clampDepositPercentage(value)),
    )
  }
  ...
  const subSteps = useMemo<SubStepDef<Partial<Tour>>[]>(
    () => [
      ...
      {
        id: 'accommodation',
        title: 'Room sharing & hotel stay',
        description:
          'Optional. For multi-day trips, set a per-person price for each room-sharing option.',
        optional: true,
      },
      {
        id: 'seasonal',
    ...
    <div className="space-y-6">
      ...
      >
            ...
            <div className="space-y-5">
                ...
                <div className="flex items-center justify-between gap-3">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-muted-foreground">
                      Choose what percentage the traveler must pay now. {membershipTierLabel}{' '}
                      requires at least {effectiveMinimumDeposit}%.
                    </p>
                  </div>
                ...
                <div className="mt-4 flex flex-wrap gap-2">
                  ...
                  <div className="flex items-center gap-2 rounded-full border border-border bg-background px-3 py-2">
                    ...
                    <Input
                      ...
                      onChange={(event) =>
                        handleDepositPercentageChange(Number(event.target.value || 0))
                      }
              ...
              {isDepositBelowTierMinimum ? (
                <div className="rounded-2xl border border-destructive/20 bg-destructive/10 p-4 text-sm text-destructive">
                  {membershipTierLabel} membership requires at least {effectiveMinimumDeposit}%
                  upfront. Increase the deposit percentage or upgrade tiers before continuing.
                </div>
              ) : null}
                ...
                <div className="mt-4 grid gap-3 md:grid-cols-2">
                  <div className="rounded-2xl border border-border/60 bg-muted/30 p-4">
                    ...
                    <p className="mt-2 text-xl font-black text-foreground">
                      {data.currency || 'PKR'} {formatMoney(standardDepositPreview.upfrontAmount)}{' '}
                      per traveler
                    </p>
                  </div>
                  <div className="rounded-2xl border border-border/60 bg-muted/30 p-4">
                    ...
                    <p className="mt-2 text-xl font-black text-foreground">
                      {data.currency || 'PKR'} {formatMoney(standardDepositPreview.remainingAmount)}{' '}
                      per traveler
                    </p>
                  </div>
                </div>
                <p className="mt-4 text-sm text-muted-foreground">
                  Collection timing: Remaining balance will be paid directly to the tour operator
                  before departure.
                </p>
              </div>
                        ...
                        <div className="flex items-center justify-between gap-3">
                          <div>
                            ...
                            <p className="text-xs text-muted-foreground">
                              {tier.minPeople}+ travelers · {data.currency || 'PKR'}{' '}
                              {formatMoney(payment.effectiveUnitPrice)} per traveler
                            </p>
                          </div>
                          <div className="text-right text-sm">
                            <p className="font-semibold text-foreground">
                              Pay now: {data.currency || 'PKR'} {formatMoney(payment.upfrontAmount)}
                            </p>
                            <p className="text-muted-foreground">
                              Pay later: {data.currency || 'PKR'}{' '}
              ...
              <div className="rounded-2xl border border-info/20 bg-info/10 p-4 text-sm text-info">
                Only the upfront amount will be charged online. The remaining amount must be
                collected from the traveler before departure.
              </div>
        ...
        {stepId === 'accommodation' ? (
          <div className="glass-card rounded-[24px] border border-border/60 bg-background p-8 shadow-sm">
            <div className="mb-8 flex items-start justify-between">
              <div>
                <h3 className="mb-1 flex items-center gap-2 pl-1 text-[14px] font-bold uppercase tracking-widest text-foreground">
                  <BedDouble className="h-5 w-5 text-primary" /> Room sharing &amp; hotel stay
                </h3>
                <p className="pl-8 text-sm text-muted-foreground">
                  For multi-day trips, the per-person price changes with how many share a room. Set
                  the options you offer — travellers pick at booking and the total updates.
                </p>
              </div>
              <Switch
                checked={accEnabled}
                onCheckedChange={(v) => writeAccommodation({ enabled: v })}
                className="data-[state=checked]:bg-primary"
              />
            </div>
                ...
                >
                  <div className="space-y-3">
                    <p className="text-sm font-medium text-muted-foreground">
                      Price per person ({data.currency || 'PKR'}) — leave a row at 0 if you
                      don&apos;t offer it.
                    </p>
                    {ROOM_TIER_DEFS.map((tier) => (
                      <div
                        key={tier.key}
                      ...
                      >
                        ...
                        <div className="relative flex-1">
                          <div className="pointer-events-none absolute left-2 top-1/2 flex h-8 -translate-y-1/2 items-center justify-center rounded-lg border border-border bg-muted px-3 text-xs font-bold text-muted-foreground">
                            {data.currency || 'PKR'}
                          </div>
                          <Input
                            type="number"
                            min={0}
                            value={tierPriceFor(tier.key) || ''}
                            placeholder="0"
                            onChange={(e) =>
                              setTierPrice(
                                tier.key,
                            ...
                            className="h-11 w-full rounded-xl border-border bg-muted/40 pl-20 text-base font-bold focus:border-primary"
                          />
                        </div>
                      </div>
                    ))}
                  ...
                  <div className="space-y-3 rounded-2xl border border-border/60 bg-muted/20 p-4">
                    <p className="text-sm font-medium text-muted-foreground">
                      Child rates — a % of the adult price for the chosen sharing tier.
                    </p>
                    <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
                      {(
                        [
                          { key: 'withBed', label: 'Child (with bed)' },
                          { key: 'noBed', label: 'Child (no bed)' },
                        ...
                        <div key={c.key} className="space-y-1">
                          ...
                          <div className="relative">
                            <Input
                              ...
                              min={0}
                              max={100}
                              value={Math.round(accChildRates[c.key] * 100)}
                              onChange={(e) => setChildRatePct(c.key, Number(e.target.value))}
                              className="h-11 w-full rounded-xl border-border bg-background pr-9 text-base font-bold focus:border-primary"
                            />
      ...
      <Dialog open={isDepositDialogOpen} onOpenChange={setIsDepositDialogOpen}>
        <DialogContent className="rounded-3xl border-border/60 bg-background/95 sm:max-w-xl">
          <DialogHeader>
            ...
            <DialogDescription>
              Only {normalizedDepositPercentage}% of the booking amount will be charged online at
              the time of booking confirmation. The remaining {100 - normalizedDepositPercentage}%
              must be collected from the traveler by the tour operator before departure.
            </DialogDescription>
          </DialogHeader>
