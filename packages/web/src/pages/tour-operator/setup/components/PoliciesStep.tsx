    ...
    return (
        <div className="space-y-12">
            <div>
                ...
                <p className="text-lg text-gray-500 leading-relaxed font-medium">Define how you operate and ensure a safe experience for travelers.</p>
            </div>
            ...
            <section className="space-y-6">
                <div className="flex items-center gap-3 mb-6">
                    ...
                    <h4 className="font-black text-gray-900 uppercase tracking-widest text-xs italic">Platform Agreement</h4>
                </div>
                <div className="space-y-4">
                    {PLATFORM_TERMS.map((term) => (
                        <Card key={term.id} className="border-gray-100 rounded-[28px] overflow-hidden bg-white shadow-sm ring-1 ring-black/[0.02] transition-all hover:shadow-md">
                            <button
                            ...
                            >
                                <div className="flex items-center gap-5">
                                    ...
                                    <div className="text-left">
                                        ...
                                        <p className="text-xs text-gray-500 font-bold uppercase tracking-widest">{term.desc}</p>
                                    </div>
                                </div>
                                <ChevronDown className={cn("w-6 h-6 text-gray-300 transition-transform duration-300 group-hover:text-primary", expanded === term.id && "rotate-180 text-primary")} />
                            </button>
                            <AnimatePresence>
                                {expanded === term.id && (
                                    <motion.div
                                    ...
                                    >
                                        <div className="p-8 text-sm text-gray-600 space-y-4 leading-relaxed bg-gray-50/30 font-medium">
                                            ...
                                            <ul className="space-y-2">
                                                {['Maintain up-to-date availability calendars.', 'Honor all confirmed bookings.', 'Respond to traveler inquiries within 24 hours.'].map((item, i) => (
                ...
                <Card
                    ...
                    className={cn(
                        ...
                        accepted ? "border-primary bg-primary/[0.03] shadow-lg shadow-primary/5" : "border-gray-50 bg-white hover:border-gray-200"
                    )}
                >
                    <div className={cn(
                        "w-8 h-8 rounded-xl flex items-center justify-center border-2 transition-all duration-300",
                        accepted ? "bg-primary border-primary text-white scale-110 shadow-lg shadow-primary/30" : "border-gray-200 text-transparent group-hover:border-primary/30"
                    )}>
                        <Check className="w-5 h-5 stroke-[3]" />
                    </div>
                    <div>
                        <p className="font-extrabold text-gray-900 tracking-tight">I accept the Platform Terms</p>
                        <p className="text-xs text-gray-500 font-bold uppercase tracking-widest leading-none mt-1">I have read and agree to all platform operating policies.</p>
                    </div>
                </Card>
            </section>
            ...
            <section className="space-y-8">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
                    <div className="flex items-center gap-3">
                        ...
                        <h4 className="font-black text-gray-900 uppercase tracking-widest text-xs italic">Your Operation Policies</h4>
                    </div>
                    <div className="flex bg-gray-100 p-1.5 rounded-2xl w-fit">
                        <button
                            ...
                            className={cn(
                                ...
                                policyMode === 'templates' ? "bg-white text-primary shadow-sm" : "text-gray-400 hover:text-gray-600"
                            )}
                        >
                        ...
                        <button
                            ...
                            className={cn(
                                "px-6 py-2 rounded-xl text-xs font-black transition-all flex items-center gap-2 uppercase tracking-widest",
                                policyMode === 'upload' ? "bg-white text-primary shadow-sm" : "text-gray-400 hover:text-gray-600"
                            )}
                        >
                ...
                {policyMode === 'templates' ? (
                            ...
                            <div key={policy.id} className="space-y-4">
                                <div className="flex items-center justify-between">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            <Label className="text-lg font-extrabold text-gray-900 tracking-tight">{policy.title}</Label>
                                            <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest leading-none mt-1">Required Information</p>
                                        </div>
                                    </div>
                    ...
                    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                        <div className="p-16 border-2 border-dashed border-gray-100 rounded-[48px] bg-gray-50/30 flex flex-col items-center justify-center text-center group hover:border-primary/20 hover:bg-primary/[0.01] transition-all duration-500">
                            ...
                            <h5 className="text-xl font-black text-gray-900 mb-2 uppercase italic tracking-tighter">Upload Policy Documents</h5>
                            <p className="text-sm text-gray-500 max-w-[280px] leading-relaxed mb-8 font-medium">
                                Upload your PDF or Word documents containing all your tour policies for our compliance team to review.
                            </p>
                                ...
                                <Card key={id} className={cn(
                                    ...
                                    <div className="flex items-center gap-5">
                                        <div className={cn(
                                            ...
                                            uploads[id] ? "bg-primary text-white rotate-6" : "bg-gray-100 text-gray-300 group-hover:text-gray-400"
                                        )}>
                                            <FileText className="w-7 h-7" />
                                        </div>
                                        <div className="text-left">
                                            <p className="font-extrabold text-gray-900 tracking-tight">{id.split('_').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(' ')}</p>
                                            <p className={cn(
                                                "text-[10px] uppercase tracking-widest font-black mt-1",
                                                uploads[id] ? "text-primary" : "text-gray-400"
                                            )}>{uploads[id] ? 'Verified' : 'Required'}</p>
                                        </div>
                                    </div>
                                    {!uploads[id] ? (
