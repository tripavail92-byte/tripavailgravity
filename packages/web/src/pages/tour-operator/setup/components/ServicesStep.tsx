    ...
    return (
        <div className="space-y-10">
            <div>
                ...
                <p className="text-lg text-gray-500 leading-relaxed font-medium">What types of tours do you specialize in? Select all that apply.</p>
            </div>
                ...
                {SERVICES.map((s) => {
                    ...
                    return (
                        <motion.button
                            ...
                            onClick={() => toggle(s.id)}
                            whileTap={{ scale: 0.98 }}
                            className={`p-8 rounded-[32px] border-2 text-left transition-all relative group flex flex-col items-center text-center space-y-4 ${isSelected
                                ? 'border-primary bg-primary/5 shadow-xl shadow-primary/5'
                                : 'border-gray-100 bg-white hover:border-gray-200 hover:shadow-lg hover:shadow-black/5'
                                }`}
                            aria-pressed={isSelected}
                        >
                            <div className={`w-16 h-16 rounded-2xl flex items-center justify-center transition-all duration-300 ${isSelected ? 'bg-primary text-white scale-110 shadow-lg shadow-primary/30' : 'bg-gray-50 text-gray-400 group-hover:bg-primary/10 group-hover:text-primary group-hover:scale-105'
                                }`}>
                                <s.icon className={`${isSelected ? 'w-8 h-8' : 'w-7 h-7'}`} aria-hidden="true" />
                            </div>
                            <div className="space-y-1">
                                <p className={`font-black tracking-tight text-lg uppercase italic transition-colors ${isSelected ? 'text-primary' : 'text-gray-900'}`}>{s.name}</p>
                                <p className="text-xs text-gray-500 font-bold uppercase tracking-widest leading-none">{s.desc}</p>
                            </div>
                            {isSelected && (
            ...
            <div className="space-y-6 pt-10 border-t border-gray-100">
                <div className="flex items-center justify-between">
                    <div className="space-y-1">
                        <h4 className="text-xl font-extrabold text-gray-900 tracking-tight">Custom Categories</h4>
                        <p className="text-sm text-gray-500 font-medium">Add any other niche specialties you offer.</p>
                    </div>
                    {!isAdding && (
                        <Button
                            variant="outline"
                ...
                {isAdding && (
                    ...
                    >
                        <Input
                            ...
                            autoFocus
                        />
                        <Button onClick={addCustom} className="rounded-2xl px-8 h-14 font-bold">Add</Button>
                        <Button variant="ghost" onClick={() => setIsAdding(false)} className="rounded-2xl h-14 font-bold text-gray-500">Cancel</Button>
                    </motion.div>
                )}

                <div className="flex flex-wrap gap-3">
                    {custom.map(c => (
                        <span key={c} className="inline-flex items-center gap-2 px-6 py-3 bg-white text-primary rounded-2xl text-sm font-bold border-2 border-primary/10 shadow-sm transition-all hover:border-primary/30">
                            {c}
                            <button onClick={() => removeCustom(c)} className="w-6 h-6 flex items-center justify-center rounded-lg hover:bg-red-50 hover:text-red-500 transition-colors" aria-label={`Remove ${c}`}>
                                <X className="w-4 h-4" />
                            </button>
                        </span>
                    ))}
