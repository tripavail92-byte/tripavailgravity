...
/**
 * Property descriptions generated by a language model, with templates as the fallback.
 *
 * THIS USED TO BE FAKE. generateSuggestions() awaited `setTimeout(1500)` — a hardcoded delay
 * dressed up as thinking — then returned one of three canned strings per property type, under a
 * "Generating…" spinner and a Wand2 icon. It told the partner a model had written something for
 * them when nothing had run at all.
 *
 * It now calls the generate-listing-copy edge function (OpenAI / gpt-4o-mini, the provider and key
 * already used by whatsapp-webhook), passing the property's real attributes. The templates below
 * are KEPT DELIBERATELY as the fallback for when the endpoint is unavailable — and when they are
 * used, the UI says so instead of passing them off as generated.
 */

interface PropertyDescriptionAIProps {
  propertyType: string
  hotelName?: string
  /** Passed to the model so the copy can mention the real place. */
  city?: string
  country?: string
  starRating?: number
  amenities?: string[]
  ...
  const generateSuggestions = async () => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke('generate-listing-copy', {
        body: {
          kind: 'property',
          property: { propertyType, name: hotelName, city, country, starRating, amenities },
        },
      })
      ...
      // Endpoint unavailable (no key, outage, timeout). Fall back to the templates rather than
      // dead-ending the wizard — but flag it, so this never again claims to be generated when it is
      // not. That claim was the original defect.
      setSuggestions(getPropertySuggestions(propertyType, hotelName))
      ...
      <AnimatePresence>
        {suggestions.length > 0 && (
          <motion.div
            ...
            className="space-y-3"
          >
            {/* The heading tracks what actually produced these. Claiming "AI-Generated" over
                template output is the exact defect this component started with. */}
            <h4 className="text-sm font-medium text-foreground flex items-center gap-2">
              <Sparkles className="w-4 h-4 text-primary" />
              {usedFallback ? 'Suggested descriptions' : 'AI-generated suggestions'}
            </h4>
            {usedFallback && (
              <p className="text-xs text-muted-foreground">
                Live generation is unavailable right now, so these are written from your property
                type. Edit them freely.
              </p>
            )}
