...
function BookingCard({
  ...
  return (
    <div className={`p-4 rounded-lg ${config.bg} border border-border`}>
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between mb-1">
            <h4 className="font-medium text-foreground">{guestName}</h4>
            <StatusIcon className={`w-4 h-4 ${config.color}`} />
          </div>

          <p className="text-sm text-muted-foreground mb-2">{roomType}</p>
      ...
      <GlassCard variant="card" className="p-6 rounded-2xl">
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">Upcoming check-ins</p>
          </div>
        ...
        {bookings.length === 0 ? (
          <div className="py-10 text-center">
            ...
            <p className="text-xs text-muted-foreground mt-1">New reservations will appear here.</p>
          </div>
        ) : (
