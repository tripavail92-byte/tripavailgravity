import { ArrowLeft, ArrowRight, Check, HelpCircle, Play, Target, X } from 'lucide-react'
...
export const GuidedTour = ({ tour, isActive, onComplete, onSkip }: GuidedTourProps) => {
  ...
  const findTargetElement = useCallback(() => {
    ...
    if (element) {
      ...
      // Adjusted scroll block to 'center' ensures visibility
  ...
  useEffect(() => {
    if (isActive) {
      ...
      console.log(`[Tour Analytics] Started tour: ${tour.id} (v${tour.version || '1.0.0'})`)
    } else {
      setIsVisible(false)
  ...
  }, [isActive, currentStepIndex, findTargetElement, tour.id, tour.version])
  ...
  const calculateTooltipPosition = useCallback(() => {
    ...
    // Keep tooltip within viewport bounds (Mobile Safe)
  ...
  const completeTour = useCallback(() => {
    setIsVisible(false)
    document.body.style.overflow = 'unset'
    console.log(`[Tour Analytics] Completed tour: ${tour.id}`)
    onComplete()
    tour.onComplete?.()
  }, [onComplete, tour])
  ...
  const skipTour = useCallback(() => {
    setIsVisible(false)
    document.body.style.overflow = 'unset'
    console.log(`[Tour Analytics] Skipped tour: ${tour.id} at step ${currentStepIndex}`)
    onSkip()
    tour.onSkip?.()
  }, [currentStepIndex, onSkip, tour])
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.key === 'Escape' && isVisible) skipTour()
    }
  ...
  }, [targetElement, currentStep, isVisible, calculateTooltipPosition, skipTour])
          ...
          >
            ...
            {targetElement && currentStep.highlight && (
              <motion.div
                ...
                animate={{
                  boxShadow: ['0 0 0 0 #E11D4840', '0 0 0 20px #E11D4820', '0 0 0 0 #E11D4840'],
                }}
          ...
          >
            <Card className="p-6 w-80 shadow-2xl border-2 border-[#E11D48]/20">
              ...
              <div className="flex items-start justify-between mb-4">
                <div className="flex items-center gap-2">
                  <div className="w-8 h-8 bg-[#E11D48] rounded-full flex items-center justify-center">
                    <span className="text-white text-sm font-medium">{currentStepIndex + 1}</span>
                  </div>
                  <div>
                ...
                <Button variant="ghost" size="sm" onClick={skipTour} className="p-1 h-6 w-6">
                  <X className="w-4 h-4" />
                </Button>
              </div>
              ...
              <div className="mb-6">
                ...
                {currentStep.content && <div className="mt-4">{currentStep.content}</div>}
              ...
              <div className="flex items-center justify-between">
                ...
                <Button
                  ...
                  disabled={currentStep.action === 'click' && Boolean(targetElement)}
                >
                  {isLastStep ? (
                    <>
