...
-- RLS Policies: Travelers view own bookings
CREATE POLICY "Travelers can view own package bookings" ON public.package_bookings
    FOR SELECT
    USING (auth.uid() = traveler_id);

-- Package owners can view bookings for their packages
CREATE POLICY "Package owners can view bookings for their packages" ON public.package_bookings
    FOR SELECT
    USING (
...
CREATE POLICY "Travelers can create package bookings" ON public.package_bookings
    FOR INSERT
    WITH CHECK (auth.uid() = traveler_id);

-- Travelers can update their own bookings (e.g., cancel)
CREATE POLICY "Travelers can update own package bookings" ON public.package_bookings
    FOR UPDATE
    USING (auth.uid() = traveler_id);
...
CREATE INDEX idx_package_bookings_created_at ON public.package_bookings(created_at DESC);
