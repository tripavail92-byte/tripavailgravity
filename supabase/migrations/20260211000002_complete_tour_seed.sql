-- Complete tour seed with schedules - all in one migration
-- This will create tours if they don't exist, then create schedules
...
BEGIN
    ...
    IF v_operator_id IS NOT NULL THEN
        ...
        FROM public.tours
        WHERE operator_id = v_operator_id AND title LIKE '%Grand Canyon%';
        
        -- Only insert tours if they don't exist
        IF v_tour_count = 0 THEN
            RAISE NOTICE 'Creating 5 sample tours...';
            ...
            (
                ...
                95,
                'USD',
                'Discover authentic Tokyo through its incredible street food scene.',
                'Authentic street food tasting tour in Tokyo',
                '["https://images.unsplash.com/photo-1540959733332-eab4deabeeaf?w=800"]'::jsonb,
                ARRAY['Visit 6+ food stops', 'Try authentic local dishes'],
                ARRAY['Food tastings at 6 venues', 'English-speaking guide'],
                ARRAY['Additional food/drinks'],
                ARRAY['Come hungry!', 'Comfortable walking shoes'],
                2, 10, 10, 99,
                'easy',
                ARRAY['English', 'Japanese'],
                true,
                '[{"id": "1", "name": "Individual", "minPeople": 1, "maxPeople": 1, "pricePerPerson": 95}]'::jsonb,
            ...
            (
                ...
                'adventure',
                '{"city": "Interlaken", "country": "Switzerland"}'::jsonb,
                '3 hours',
                199,
                'USD',
                'Soar above the stunning Swiss Alps.',
                'Tandem paragliding over the Swiss Alps',
                '["https://images.unsplash.com/photo-1519904981063-b0cf448d479e?w=800"]'::jsonb,
                ARRAY['15-20 minute tandem flight', 'Certified instructors'],
                ARRAY['Tandem paragliding flight', 'All safety equipment'],
                ARRAY['Photo/video package (€40)'],
                ARRAY['Good health required', 'Weight limit: 30-115 kg'],
                1, 8, 14, 65,
                'moderate',
            ...
            (
                ...
                'cultural',
                '{"city": "Rome", "country": "Italy"}'::jsonb,
                '3.5 hours',
                65,
                'USD',
                'Step back in time and explore ancient Rome.',
                'Guided tour of ancient Rome''s iconic landmarks',
                '["https://images.unsplash.com/photo-1552832230-c0197dd311b5?w=800"]'::jsonb,
                ARRAY['Skip-the-line Colosseum access', 'Expert historian guide'],
                ARRAY['Colosseum entrance ticket', 'Roman Forum access'],
                ARRAY['Food and drinks'],
                ARRAY['Comfortable walking shoes'],
                2, 15, 8, 99,
                'easy',
                ARRAY['English', 'Italian', 'Spanish'],
                true,
                '[{"id": "1", "name": "Adult", "minPeople": 1, "maxPeople": 99, "pricePerPerson": 65}]'::jsonb,
                false, 1.0, 1.0, false, 0,
                'flexible',
            ...
            (
                ...
                'nature',
                '{"city": "Ubud", "country": "Indonesia"}'::jsonb,
                '8 hours',
                85,
                'USD',
                'Discover Bali''s natural wonders.',
                'Full-day nature and culture tour in Bali',
                '["https://images.unsplash.com/photo-1537996194471-e657df975ab4?w=800"]'::jsonb,
                ARRAY['3 stunning waterfalls', 'Tegalalang Rice Terraces'],
                ARRAY['Hotel pickup/dropoff', 'Private air-conditioned vehicle'],
                ARRAY['Personal expenses'],
                ARRAY['Modest dress for temples'],
                1, 6, 5, 75,
                'easy',
                ARRAY['English', 'Indonesian'],
                true,
                '[{"id": "1", "name": "Private (1-2 pax)", "minPeople": 1, "maxPeople": 2, "pricePerPerson": 85}]'::jsonb,
                false, 1.0, 1.0, false, 0,
                'moderate',
                4.8, 178, true, true, true, true
            );
            
            RAISE NOTICE 'Created 5 sample tours';
        ELSE
            RAISE NOTICE 'Tours already exist, skipping creation';
        END IF;
        
        -- Delete old schedules and create fresh ones for ALL published tours
        DELETE FROM public.tour_schedules
        WHERE tour_id IN (
            SELECT id FROM public.tours WHERE operator_id = v_operator_id AND is_published = true
        );
        
        RAISE NOTICE 'Creating schedules for all published tours...';
        ...
        RAISE NOTICE 'Successfully added tours and schedules';
    ELSE
