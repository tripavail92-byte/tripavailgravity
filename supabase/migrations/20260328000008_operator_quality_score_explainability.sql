-- Admin-only operator quality score explainability for ranking calibration.
...
BEGIN
  ...
  explained AS (
    SELECT
      scored.*,
      'operator_quality_v1'::TEXT AS score_policy_version,
      jsonb_build_object(
        'review_quality', to_jsonb(ARRAY_REMOVE(ARRAY[
          CASE WHEN scored.total_reviews = 0 THEN 'no_reviews_yet' END,
          CASE WHEN scored.total_reviews > 0 AND scored.total_reviews < 5 THEN 'low_review_volume' END,
          CASE WHEN scored.total_reviews >= 20 THEN 'established_review_volume' END,
          CASE WHEN scored.avg_rating >= 4.50 THEN 'strong_review_average' END,
          CASE WHEN scored.avg_rating > 0 AND scored.avg_rating < 4.00 THEN 'rating_needs_attention' END
        ], NULL)::TEXT[]),
        'verification', to_jsonb(ARRAY_REMOVE(ARRAY[
          CASE WHEN scored.verified_badge_count = 0 THEN 'no_verified_documents' END,
          CASE WHEN scored.verified_badge_count BETWEEN 1 AND 2 THEN 'partial_verification_coverage' END,
          CASE WHEN scored.verified_badge_count >= 3 THEN 'strong_verification_coverage' END
        ], NULL)::TEXT[]),
        'responsiveness', to_jsonb(ARRAY_REMOVE(ARRAY[
          CASE WHEN scored.response_rate = 0 THEN 'no_reply_data' END,
          CASE WHEN scored.response_rate >= 80 THEN 'high_reply_rate' END,
          CASE WHEN scored.response_rate > 0 AND scored.response_rate < 50 THEN 'low_reply_rate' END,
          CASE WHEN scored.avg_response_minutes > 0 AND scored.avg_response_minutes <= 60 THEN 'fast_response_time' END,
          CASE WHEN scored.avg_response_minutes > 1440 THEN 'slow_response_time' END
        ], NULL)::TEXT[]),
        'reliability', to_jsonb(ARRAY_REMOVE(ARRAY[
          CASE WHEN scored.cancellation_rate >= 10 THEN 'elevated_cancellation_risk' END,
          CASE WHEN scored.open_report_count > 0 THEN 'open_concerns_present' END,
          CASE WHEN scored.cancellation_rate < 5 AND scored.open_report_count = 0 THEN 'clean_operational_record' END
        ], NULL)::TEXT[]),
        'completeness', to_jsonb(ARRAY_REMOVE(ARRAY[
          CASE WHEN scored.storefront_completion_rate >= 90 THEN 'complete_storefront' END,
          CASE WHEN scored.storefront_completion_rate >= 60 AND scored.storefront_completion_rate < 90 THEN 'storefront_has_minor_gaps' END,
          CASE WHEN scored.storefront_completion_rate < 60 THEN 'storefront_needs_work' END
        ], NULL)::TEXT[]),
        'performance', to_jsonb(ARRAY_REMOVE(ARRAY[
          CASE WHEN scored.booking_starts = 0 THEN 'no_booking_start_data' END,
          CASE WHEN scored.booking_starts > 0 AND scored.booking_starts < 3 THEN 'sparse_funnel_data' END,
          CASE WHEN scored.engagement_rate >= 40 THEN 'strong_profile_engagement' END,
          CASE WHEN scored.attributed_booking_starts > 0 THEN 'conversion_after_profile_view' END
        ], NULL)::TEXT[])
      ) AS score_reason_codes,
      jsonb_build_object(
        'review_volume_confidence', CASE
          WHEN scored.total_reviews >= 20 THEN 'high'
          WHEN scored.total_reviews >= 5 THEN 'medium'
          ELSE 'low'
        END,
        'response_time_bucket', CASE
          WHEN scored.avg_response_minutes <= 0 THEN 'none'
          WHEN scored.avg_response_minutes <= 60 THEN 'within_1_hour'
          WHEN scored.avg_response_minutes <= 720 THEN 'same_day'
          WHEN scored.avg_response_minutes <= 1440 THEN 'within_24_hours'
          WHEN scored.avg_response_minutes <= 2880 THEN 'within_48_hours'
          ELSE 'over_48_hours'
        END,
        'reliability_pressure', CASE
          WHEN scored.open_report_count > 0 OR scored.cancellation_rate >= 10 THEN 'high'
          WHEN scored.cancellation_rate >= 5 THEN 'medium'
          ELSE 'low'
        END,
        'storefront_completeness_band', CASE
          WHEN scored.storefront_completion_rate >= 90 THEN 'strong'
          WHEN scored.storefront_completion_rate >= 60 THEN 'moderate'
          ELSE 'weak'
        END,
        'performance_data_band', CASE
          WHEN scored.booking_starts >= 5 THEN 'established'
          WHEN scored.booking_starts >= 1 THEN 'emerging'
          ELSE 'none'
        END,
        'minimum_data_warning', (scored.total_reviews < 5 OR scored.booking_starts < 3),
        'avg_rating_input', scored.avg_rating,
        'total_reviews_input', scored.total_reviews,
        'verified_badge_input', scored.verified_badge_count,
        'response_rate_input', scored.response_rate,
        'avg_response_minutes_input', scored.avg_response_minutes,
        'cancellation_rate_input', scored.cancellation_rate,
        'open_report_count_input', scored.open_report_count,
        'total_report_count_input', scored.total_report_count,
        'storefront_completion_rate_input', scored.storefront_completion_rate,
        'engagement_rate_input', scored.engagement_rate,
        'attributed_conversion_rate_input', scored.attributed_conversion_rate,
        'booking_starts_input', scored.booking_starts,
        'attributed_booking_starts_input', scored.attributed_booking_starts
  ...
  SELECT
    ...
    ROUND(
      explained.review_quality_score
      + explained.verification_score
      + explained.responsiveness_score
      + explained.reliability_score
      + explained.completeness_score
      + explained.performance_score,
      2
