...
CREATE OR REPLACE FUNCTION public.admin_update_operator_payout_hold(
  p_operator_user_id UUID,
  p_payout_hold BOOLEAN,
  p_reason TEXT
)
...
DECLARE
  ...
  v_previous_row public.operator_commercial_profiles;
  v_updated_row public.operator_commercial_profiles;
BEGIN
  ...
  IF v_role NOT IN ('super_admin'::public.admin_role_enum, 'finance_admin'::public.admin_role_enum) THEN
    RAISE EXCEPTION 'Insufficient privileges to update payout hold';
  END IF;
  ...
  IF v_previous_row.operator_user_id IS NULL THEN
    RAISE EXCEPTION 'Commercial profile for operator % not found', p_operator_user_id
      USING ERRCODE = 'P0002';
  END IF;

  UPDATE public.operator_commercial_profiles
  SET
    payout_hold = p_payout_hold,
    payout_hold_reason = CASE
      WHEN p_payout_hold THEN v_reason
      ELSE NULL
    END,
  ...
  PERFORM public.admin_log_action(
    ...
    p_operator_user_id,
    CASE
      WHEN p_payout_hold THEN 'payout_hold_applied'
      ELSE 'payout_hold_released'
    END,
    v_reason,
    jsonb_build_object(
      'payout_hold', v_previous_row.payout_hold,
      'payout_hold_reason', v_previous_row.payout_hold_reason
    ),
    jsonb_build_object(
      'payout_hold', v_updated_row.payout_hold,
      'payout_hold_reason', v_updated_row.payout_hold_reason
    )
  );
...
BEGIN
  ...
  SELECT payout_item.booking_id, payout_item.recovery_amount, payout_item.payout_status
  INTO v_booking_id, v_existing_recovery_amount, v_previous_status
  FROM public.operator_payout_items AS payout_item
...
GRANT EXECUTE ON FUNCTION public.admin_update_operator_payout_hold(UUID, BOOLEAN, TEXT) TO authenticated;
