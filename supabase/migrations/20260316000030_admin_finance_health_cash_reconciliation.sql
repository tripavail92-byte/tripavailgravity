  ...
  SELECT
    ROUND(
      COALESCE(
        ...
        FILTER (
          WHERE settlement_state = 'paid_pending_service'::public.settlement_state_enum
            AND payout_status = 'not_ready'::public.payout_status_enum
        ),
        0
      ),
      2
    ) AS total_operator_liability_pending_service,
    ROUND(
      COALESCE(
        SUM(GREATEST(payment_collected - refund_amount - COALESCE(commission_collected, commission_amount), 0))
...
SELECT
  ...
  snapshot_cash_buckets.total_operator_liability_pending_service,
