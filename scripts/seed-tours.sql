-- Quick tour seeding script that creates sample tours directly
-- Run this in Supabase SQL Editor

-- First, get a valid operator ID or create a placeholder
DO $$
DECLARE
    v_operator_id UUID;
BEGIN
    -- Try to get an existing tour operator
    SELECT user_id INTO v_operator_id
    FROM public.user_roles
    WHERE role_type = 'tour_operator'
    LIMIT 1;
    
    -- If no operator exists, use a placeholder ID
    IF v_operator_id IS NULL THEN
        v_operator_id := '00000000-0000-0000-0000-000000000001'::UUID;
    END IF;
    
    -- Insert 5 sample tours
    ...
    -- Add schedules for each tour (next 14 days)
    ...
    RAISE NOTICE 'Successfully seeded % tours with schedules', (SELECT COUNT(*) FROM tours WHERE operator_id = v_operator_id AND created_at > NOW() - interval '1 minute');
END $$;

-- Verify the results
SELECT COUNT(*) as tour_count FROM public.tours WHERE is_active = true;
SELECT COUNT(*) as schedule_count FROM public.tour_schedules WHERE status = 'scheduled';
